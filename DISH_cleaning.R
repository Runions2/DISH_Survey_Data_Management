...
df.intake24_item <- df.intake24_item %>%
  select(-proxy, -proxy_issues, -serving_image, -leftovers_image, -food_group_local, -brand, -meal_index, 
         -meal_id, -food_index, -nutrient_table_name, -reasonable_amount) 
...
# ethnicity
# code removed to prevent identifiability
...
df.ndb <- df.ndb %>%
  rename(Description_English = food_name,
         NutrientTableCode= food_number)
...
df.missing_foods <- df.missing_foods %>%
  mutate(across(45:131, ~ as.numeric(.)))

df.missing_foods <- df.missing_foods %>%
  mutate(across(45:131, ~ . * (Estimated_Portion_Size_g / 100)))
...
df.missing_foods <- df.missing_foods %>%
  ...
  rename(
    ...
    Fatg = fat,
    Alcoholg = alco,
    Saturatedfattyacidsg = satfa,
    Transfattyacidsg = trans,
    Freesugarsg = freesug,
    AOACg = aoac,
    VitaminAug = vita,
    Riboflavinmg = ribo,
    ...
    Lambg = lamb,
    Porkg = pork,
    ProcessedRedMeatg = process_red_meat,
    OtherRedMeatg = other_red_meat,
    Burgersg = burgers,
    Sausagesg = sausages,
    Offalg = offal,
    ...
    CheddarCheeseg = cheddar_cheese,
    OtherCheeseg = other_cheese,
    #add in values for data archive
    Water = water, 
    TotalNitrogen = totnit, 
    NitrogenConversionFactor = ncf, 
    Starch = star, 
    Glucose = gluc, 
    Fructose = fruct, 
    Maltose = malt, 
    ...
    Cholesterol = chol, 
    Retinol = ret, 
    TotalCarotene = totcar, 
    AlphaCarotene = acar, 
    BetaCarotene = bcar, 
    BetaCryptoxanthin = bcrypt, 
    NiacinEquivalent = niacequ, 
    Copper = cu, 
    Chloride = cl, 
    Manganese = mn, 
    Thiamin = thia, 
