...
# Import the data
```{r include=FALSE}
#df.intake24_item <- read.csv( "/Users/ricki/Desktop/DISH_data_cleaning/Output/intake24_item_110924.csv") 
#df.survey <- read.csv( "/Users/ricki/Desktop/DISH_data_cleaning/Output/survey_clean_110924.csv") 
...
## **TO CHECK** Clean high and low energy recalls
...
#write.xlsx(high_kcal_items, file = "/Users/ricki/Desktop/DISH_data_cleaning/Output/intake24_high_kcal_all_110924.xlsx")
...
df.intake24_item <- df.intake24_item %>%
  filter(!(UserID == 10002909 & RecallNo == 1) &
           !UserID == 10006095 & RecallNo %in% c(1,2) & #where is recall 3???
           !(UserID == 10001709 & RecallNo == 3))

df.intake24_recall <- df.intake24_recall %>%
  filter(!(UserID == 10002909 & RecallNo == 1) &
           !UserID == 10006095 & RecallNo %in% c(1,2) &
           !(UserID == 10001709 & RecallNo == 3))
...
check %>% filter(UserID == 10006095) %>%
  select(UserID, RecallNo, NumberOfRecalls)
```
...
df.intake24_item <- df.intake24_item %>%
  filter(!(UserID == 10009887 & RecallNo == 1) &
           ...
           !UserID == 20002885 & #removing participant as all recalls did not pass quality checks
...
df.intake24_recall <- df.intake24_recall %>%
  filter(!(UserID == 10009887 & RecallNo == 1) &
           !(UserID == 10000459 & RecallNo == 1) &
           !UserID == 20002885 & #removing participant as all recalls did not pass quality checks
           !(UserID == 30000069 & RecallNo == 4)) #where is recall 3???
...
df.intake24_item %>% filter(UserID == 30000069) %>%
  select(UserID, RecallNo, NumberOfRecalls)
```

### Adjust Nutrient information
```{r eval=FALSE, include=FALSE}
...
## Re-code daily nutrients 
Need to recalculate daily intakes after energy intake adjustments
### Total daily nutrient and food group intake (across items)
```{r echo=FALSE, message=FALSE, warning=FALSE}
  df.intake24_recall <- df.intake24_item %>% 
      group_by(UserID, RecallNo) %>%
...
# Contributions of NDNS food groups to nutrient intake
## Total daily energy and nutrient intakes from food groups
```{r}
...
df.intake24_recall_ndns_fg <- left_join(df.intake24_recall_ndns_fg, df.numberrecalls, by = "UserID") 
...
## Percent energy and nutrients from food groups 
```{r}
...
df.intake24_recall_ndns_fg <- left_join(df.intake24_recall_ndns_fg, df.day_nutrients, by=c("UserID", "RecallNo"))
...
# Add in Main Food Group description variable
    df.intake24_recall_ndns_fg <- df.intake24_recall_ndns_fg %>%
      mutate(MainFoodGroupDesc = case_when(
        ...
        TRUE ~ NA)) %>%
  filter(!is.na(MainFoodGroupDesc)) %>%
  relocate(MainFoodGroupDesc, .after = MainFoodGroupCode)
```

```{r}
# pivot to wide format
df.intake24_participant_ndns_fg_wide <- df.intake24_recall_ndns_fg %>% 
  select(-MainFoodGroupDesc)   
...
# Contributions of food categories to nutrient intake
## Total daily energy and nutrient intakes from food groups
```{r}
df.intake24_recall_foodcat_fg <- df.intake24_item %>%
...
# Join daily intakes with expanded grids
df.intake24_recall_foodcat_fg <- left_join(foodgroup_grid, df.intake24_recall_foodcat_fg, by = c("UserID", "RecallNo", "FoodCatCode")) %>%
  select(-NumberOfRecalls)

# Fill in NumberOfRecalls
df.numberrecalls <- df.intake24_participant %>% select(UserID, NumberOfRecalls)
df.intake24_recall_foodcat_fg <- left_join(df.intake24_recall_foodcat_fg, df.numberrecalls, by = "UserID") 
df.intake24_recall_foodcat_fg <- df.intake24_recall_foodcat_fg %>%
  relocate(NumberOfRecalls, .after=UserID)
...
df.intake24_recall_foodcat_fg <- left_join(df.intake24_recall_foodcat_fg, df.day_nutrients, by=c("UserID", "RecallNo"))
...
df.intake24_recall_foodcat_fg <- df.intake24_recall_foodcat_fg %>%
  ...
  select(UserID, NumberOfRecalls, RecallNo, FoodCatCode, Day_Prop_Energykcal_fg:Day_Prop_Zincmg_fg)  %>%  
  ungroup() 
```
...
```{r}
# Calculate daily intakes of energy, fat, saturated fat, free sugars
df.intake24_recall_discretionary_fg <- df.intake24_item %>%
...
# Join daily intakes with expanded grids
df.intake24_recall_discretionary_fg <- left_join(foodgroup_grid, df.intake24_recall_discretionary_fg, by = c("UserID", "RecallNo", "DiscretionaryFoodGroupCode")) %>%
  select(-NumberOfRecalls)

# Fill in NumberOfRecalls
df.intake24_recall_discretionary_fg <- left_join(df.intake24_recall_discretionary_fg, df.numberrecalls, by = "UserID") 
df.intake24_recall_discretionary_fg <- df.intake24_recall_discretionary_fg %>%
  relocate(NumberOfRecalls, .after=UserID)
...
df.intake24_recall_discretionary_fg <- left_join(df.intake24_recall_discretionary_fg, df.day_nutrients, by=c("UserID", "RecallNo"))
...
# pivot to wide format
df.intake24_recall_discretionary_fg_wide <- df.intake24_recall_discretionary_fg %>%
  pivot_wider(
...
```{r include=FALSE}
df.intake24_participant_discretionary_fg_wide <- df.intake24_recall_discretionary_fg_wide

#write.csv(df.intake24_participant, file = "/Users/ricki/Desktop/DISH_data_cleaning/Output_data_analysis/intake24_participant_110924.csv")
#write.csv(df.intake24_recall, file = "/Users/ricki/Desktop/DISH_data_cleaning/Output_data_analysis/intake24_recall_110924.csv")
#write.csv(df.intake24_item, file = "/Users/ricki/Desktop/DISH_data_cleaning/Output_data_analysis/intake24_item_110924.csv")
#write.csv(df.survey, file = "/Users/ricki/Desktop/DISH_data_cleaning/Output_data_analysis/survey_110924.csv")

#write.csv(df.intake24_recall_ndns_fg, file = "/Users/ricki/Desktop/DISH_data_cleaning/Output_data_analysis/intake24_ndns_food_groups_110924.csv")
#write.csv(df.intake24_participant_ndns_fg_wide, file = "/Users/ricki/Desktop/DISH_data_cleaning/Output_data_analysis/intake24_ndns_food_groups_WIDE_110924.csv")
#write.csv(df.intake24_recall_discretionary_fg, file = "/Users/ricki/Desktop/DISH_data_cleaning/Output_data_analysis/intake24_discretionary_food_groups_110924.csv")
#write.csv(df.intake24_participant_discretionary_fg_wide, file = "/Users/ricki/Desktop/DISH_data_cleaning/Output_data_analysis/intake24_discretionary_food_groups_WIDE_110924.csv") 
...
write.csv(df.intake24_recall_ndns_fg, file = "C:/Users/ljaacks/OneDrive - University of Edinburgh/Scottish Diets/_National Monitoring of Diet Scotland/3_DISH/Analysis/Data//intake24_ndns_food_groups_110924.csv")
write.csv(df.intake24_participant_ndns_fg_wide, file = "C:/Users/ljaacks/OneDrive - University of Edinburgh/Scottish Diets/_National Monitoring of Diet Scotland/3_DISH/Analysis/Data//intake24_ndns_food_groups_WIDE_110924.csv")
write.csv(df.intake24_recall_discretionary_fg, file = "C:/Users/ljaacks/OneDrive - University of Edinburgh/Scottish Diets/_National Monitoring of Diet Scotland/3_DISH/Analysis/Data//intake24_discretionary_food_groups_110924.csv")
write.csv(df.intake24_participant_discretionary_fg_wide, file = "C:/Users/ljaacks/OneDrive - University of Edinburgh/Scottish Diets/_National Monitoring of Diet Scotland/3_DISH/Analysis/Data/intake24_discretionary_food_groups_WIDE_110924.csv") 
write.csv(df.intake24_recall_foodcat_fg, file = "C:/Users/ljaacks/OneDrive - University of Edinburgh/Scottish Diets/_National Monitoring of Diet Scotland/3_DISH/Analysis/Data/intake24_food_categories_WIDE_110924.csv") 
```
