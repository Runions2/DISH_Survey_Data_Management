...
df.intake24 <- read.csv("/Users/ricki/Desktop/DISH_data_cleaning/Output/intake24_clean_for_Analysis.csv") %>%
  select(-X)
...
char_not_to_convert <- c("SurveyID", "UserID", "Device", "StartTime", 
                          "FinishTime", "SurveyDuration", "FoodAmount", "FoodAmountReason", "MealIndex", "MealID", "MealName", 
                          "MealTime", "FoodSource", "FoodIndex", "SearchTerm", "FoodID", "Intake24FoodCode", 
                          "Description_English", "Description_Local", "NutrientTableName", "FoodGroupEnglish", "ReadyMeal", "NumberOfRecalls", 
                          "NumberOfItems", "date", "Sex", "CalcAge", "age_cat", "AsServedWeightFactor", 
                          "ReasonableAmount", "MissingFoodID", "MissingFoodDescription", "MissingFoodPortionSize",
                          "MissingFoodLeftovers", "SubFoodGroupCode", "NewMealName")
...
***Calculate Daily Intakes of Nutrients (g and % of food energy)***
Capture food energy (excluding alcohol) to estimate intakes of some nutrients as a % of food energy

```{r}

# Create Food Energy (kcal) variable
df.intake24 <- df.intake24 %>%
  mutate(FoodEnergy = Energykcal-(Alcoholg*7)) #7kcal per gram of alcohol
    
# Create Food grams variable
df.intake24 <- df.intake24 %>%
  mutate(FoodGrams = TotalGrams - Alcoholg)

# Calculate intakes of some nutrients as calories (kcal)
df.intake24 <- df.intake24 %>%
  mutate(
    Carbs_kcal = (Carbohydrateg*3.75),
    TotalSugars_kcal = (NONMILKSUGARS*3.75), #SUGARS
    Fat_kcal = (Fatg*9),
    SatFat_Kcal = (Saturatedfattyacidsg*9),
    TransFat_Kcal = (Transfattyacidsg*9),
    Protein_Kcal = (Proteing*3.75))
```
...
df.intake24_day <- df.intake24 %>% 
  group_by(UserID, RecallNo) %>%
  summarise(
    age = first(CalcAge), # Keeping this variable in summarised dataset
    NumberOfRecalls = first(NumberOfRecalls), # Keeping this variable in summarised dataset
    Day_Energykcal = sum(Energykcal),
    Day_EnergyKJ = sum(Energykj),
    Day_FoodEnergy = sum(FoodEnergy),
    Day_FoodGrams = sum(FoodGrams),
    Day_Carbohydrate = sum(Carbohydrateg),
    Day_TotalSugars = sum(NONMILKSUGARS), #SUGARS
...
# Calculate mean energy density
  # This calculation should be based on energy from food and milk (i.e. excluding all alcoholic and non-alcoholic beverages, but as written it only excludes alcoholic beverages (i.e. food energy). We need to exclude non-alcoholic beverages and create a new variable for 'EnergyFoodMilk'
...
df.intake24_participant <- df.intake24_participant %>%
  ...
  mutate(
    ...
    Protein_PropFoodEnergy = (Avg_Protein_Kcal/Avg_FoodEnergy)*100) %>%
  ungroup()
...
Categorising sub food groups into main food groups based on NDNS definitions
```{r}
# Note there are no main food groups 46, 60 or 61 in the NDNS
...
***Re-code milk product categories***

1. Remove dairy-free items
```{r}
...
2. Remove hot chocolate made with water
```{r}
#Sub food group
df.intake24 <- df.intake24 %>% 
  mutate(SubFoodGroupCode = case_when(
...
3. Move milky coffees into 'other milk'
```{r}
#Sub food group
df.intake24 <- df.intake24 %>% 
  mutate(SubFoodGroupCode = case_when(
...
4. Re-categorise dairy-free coffees out of 'other milk'
```{r}
#Sub food group
df.intake24 <- df.intake24 %>% 
  mutate(SubFoodGroupCode = case_when(
...
***Re-code discretionary foods following FSS Discretionary Foods Report ***
...
1) Calculate daily intakes of energy and free sugars from food groups
```{r}
...
2) Calculate mean daily intakes of energy and free sugars from food groups
```{r}
...
3) Calculate energy and free sugar contributions from food groups
```{r}
...
Create binary variables for adherence to SDGs. 
Adherence to red and processed meat, and oily fish SDG will be assessed through food frequency questions below, not Intake24.
...
# Fat
  ...
  df.intake24_participant <- df.intake24_participant%>%
    group_by(UserID) %>%
      mutate(SDG_SatFat = case_when(
        SatFat_PropFoodEnergy <= 11 ~ 1,
        SatFat_PropFoodEnergy > 11 ~ 0,
          TRUE ~ NA)) %>%
    ungroup()
  
  # Trans fat (max 1% of food energy)
  df.intake24_participant <- df.intake24_participant%>%
    group_by(UserID) %>%
      mutate(SDG_TransFat = case_when(
...
# Fibre (min 15g/d for 2-5y; min 20g/d for 5-11y; min 25g/d for 11-16y)
  # Have added in target for those >=17y also, N/A for this dataset
df.intake24_participant <- df.intake24_participant %>%
  group_by(UserID) %>%
...
SDG for fruit & veg requires more re-coding to explore intakes in grams and portions
Following portion sizes from School Lunch regs: https://www.gov.scot/publications/healthy-eating-schools-guidance-2020/pages/4/ 

```{r}
# SDG: at least 5 portions/day (≥200 g/day)
...
```{r}
#df.intake24_participant$age_cat <- as_factor(df.intake24_participant$age_cat, levels = c("2-4y", ""))
...
We will evaluate whether a participant met the SDG for oily fish and red and red processed meat via the food frequency questions from the survey
```{r}
#create age buckets
df.survey$Age <- as.numeric(df.survey$Age)
...
Create binary variables for oily fish and red meat
```{r}
#Oily fish
df.survey <- df.survey %>%
