...
#df.ndb <- read_excel( "/Users/ricki/Desktop/DISH_data_cleaning/Data/UK_NDB_1.2 2023.09.20.xlsx")
...
df.intake24_item <- df.intake24_item %>%
  filter(!UserID %in% c(10007139, 10006439, 10004147)) #10006439 in survey but did not go on to complete a recall
...
## **TO CHECK** Day of the week recall completed
```{r echo=FALSE}
df.intake24_item <- df.intake24_item %>%
...
```{r}
df.missing_foodcodes_manual <- read_excel( "/Users/ricki/Desktop/DISH_data_cleaning/Output/Missing_Food_Codes_NDB_050924.xlsx") %>%
  select(UserID, NDB_match_food_code) %>%
  filter(!is.na(NDB_match_food_code))

#add in column with manually linked ndb code
check <- left_join(df.intake24_item, df.missing_foodcodes_manual, by = "UserID", relationship = "many-to-many") %>%
  relocate(NDB_match_food_code, .after = "MissingFoodLeftovers")

check$NDB_match_food_code <- as.numeric(check$NDB_match_food_code)


#prep ndb dataset
df.ndb <- df.ndb %>%
  select( "Description_Local", "NutrientTableCode", "SubFoodGroupCode", "Proteing", "Fatg", "Carbohydrateg", "Energykcal", "Energykj", "Alcoholg" , "AOACg", "Saturatedfattyacidsg", "Transfattyacidsg", "VitaminAug", "VitaminDug", "Riboflavinmg", "VitaminCmg", "VitaminB12ug", "Folateug", "Sodiummg", "Potassiummg", "Calciummg", "Magnesiummg", "Ironmg", "haemironmg", "Nonhaemironmg", "Zincmg", "Iodineug", "Seleniumug", "Freesugarsg", "Fruitg", "DriedFruitg", "FruitJuiceg", "SmoothieFruitg", "Tomatoesg", "TomatoPureeg", "Brassicaceaeg" , "YellowRedGreeng", "Beansg", "Nutsg", "OtherVegg", "Beefg", "Lambg", "Porkg", "ProcessedRedMeatg", "OtherRedMeatg", "Burgersg", "Sausagesg", "Offalg", "Poultryg", "ProcessedPoultryg", "GameBirdsg", "WhiteFishg", "OilyFishg", "CannedTunag", "Shellfishg", "CottageCheeseg", "CheddarCheeseg", "OtherCheeseg") %>%
  mutate(NDB_match_food_code = NutrientTableCode)


columns_to_update <- c("Description_English", "Description_Local", "NutrientTableCode", "SubFoodGroupCode", "Proteing", "Fatg", "Carbohydrateg", "Energykcal", "Energykj", "Alcoholg" , "AOACg", "Saturatedfattyacidsg", "Transfattyacidsg", "VitaminAug", "VitaminDug", "Riboflavinmg", "VitaminCmg", "VitaminB12ug", "Folateug", "Sodiummg", "Potassiummg", "Calciummg", "Magnesiummg", "Ironmg", "haemironmg", "Nonhaemironmg", "Zincmg", "Iodineug", "Seleniumug", "Freesugarsg", "Fruitg", "DriedFruitg", "FruitJuiceg", "SmoothieFruitg", "Tomatoesg", "TomatoPureeg", "Brassicaceaeg" , "YellowRedGreeng", "Beansg", "Nutsg", "OtherVegg", "Beefg", "Lambg", "Porkg", "ProcessedRedMeatg", "OtherRedMeatg", "Burgersg", "Sausagesg", "Offalg", "Poultryg", "ProcessedPoultryg", "GameBirdsg", "WhiteFishg", "OilyFishg", "CannedTunag", "Shellfishg", "CottageCheeseg", "CheddarCheeseg", "OtherCheeseg")

#replace columns in intake24 with columns in NDB based on NDB_match_food_code
...
```{r include=FALSE}
#write.csv(df.intake24_item, file = "/Users/ricki/Desktop/DISH_data_cleaning/Output/intake24_item_110924.csv")
#write.csv(df.survey, file = "/Users/ricki/Desktop/DISH_data_cleaning/Output/survey_clean_110924.csv")
#write.csv(df.char, file = "/Users/ricki/Desktop/DISH_data_cleaning/Output/participant_characteristics_110924.csv")
...
write.csv(df.char, file = "C:/Users/ljaacks/OneDrive - University of Edinburgh/Scottish Diets/_National Monitoring of Diet Scotland/3_DISH/Analysis/Data/participant_characteristics_110924.csv")
```
