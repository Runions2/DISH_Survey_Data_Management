...
df.intake24_item <- df.intake24_item %>%
  select(-proxy, -proxy_issues, -serving_image, -leftovers_image, -food_group_local, -brand, -meal_index, -meal_id, -food_index, -nutrient_table_name, -reasonable_amount, -water, -total_nitrogen, -nitrogen_conversion_factor, -non_milk_extrinsic_sugars, -intrinsic_and_milk_sugars, -starch, -englyst_fibre, -glucose, -fructose, -maltose, -lactose, -sucrose, -total_sugars, -other_sugars_uk, -fs_table_sugar, -fs_other_added_sugar, -fs_honey, -fs_fruit_juice, -fs_dried_fruit, -fs_fruit_puree, -fs_stewed_fruit, -fs_vegetable_puree, -cis_mon_fa, -cis_n3_fa, -cis_n6_fa, -cholesterol, -retinol, -total_carotene, -alpha_carotene, -beta_carotene, -beta_cryptoxanthin, -tryptophan_60, -niacin_equivalent, -copper, -chloride, -manganese, -thiamin, -vitamin_e, -niacin, -vitamin_b6, -pantothenic_acid, -biotin, -phosphorus) #wait to add these back, consider adding back for final dataset for data archive
...
discretionary foods - energy, sat fat, total fat, free sugars

## Reporting Food Groups - **double check and match to slides
...
df.intake24_item <- df.intake24_item %>% 
      ...
      relocate(RedMeat, .after = GameBirdsg)
...
```{r}
#re-categorise discretionary foods *** double check we're not missing any food codes that appear in DISH but not shes and create other categories 
...
cereals <- c("8169", "10274", "11149", "10872", "8483", "8409", "206", "213", "5328", "11530", "7626", "8483", "7630", "224", "10513", "204", "205", "10512", "227", "220", "223", "10123","9032", "8169", "7630", "223", "2970", "11071", "213") #excluding poptarts (8853) from MainFoodGroup

potatoes <- c("11117", "11119", "1837", "1830", "1838", "8294", "3991", "2609", "2608", "5065")
...
df.intake24_item <- df.intake24_item %>% 
 mutate(ReportingFoodGroupCode = case_when (
   ...
   # Fruit juice - sometimes need to specify the subfoodgroup instead of main food group - this captures only fruit juice and not smoothies
  ...
  MainFoodGroupCode == 7 & NutrientTableCode %in% c(savoury_biscuits_list) ~ 11,
   # Cakes, pastries and fruit pies
   MainFoodGroupCode == 8 | MainFoodGroupCode == 9 ~ 12,
   # Crisps and savoury snacks
   MainFoodGroupCode == 42 ~ 13,
   #Pizzas
   NutrientTableCode %in% c(pizza) ~ 14,
   #ready meals
   NutrientTableCode %in% c(ready_meal) ~ 15,
   #Ice creams
   MainFoodGroupCode == 53 | NutrientTableCode %in% c(lollies) ~ 16,
   #cereals
   MainFoodGroupCode == 5 | NutrientTableCode %in% c(cereals) ~ 17,
   #Chips, fried and roast potatoes and potato products
   MainFoodGroupCode == 38 | NutrientTableCode %in% c(potatoes) ~ 18,
   #Yoghurts, fromage frais, and dairy desserts
   MainFoodGroupCode == 15 ~ 19, 
      TRUE ~ NA))
...
df.intake24_item <- df.intake24_item %>%
  mutate(ReportingFoodGroupDesc = case_when(
    ...
    relocate(ReportingFoodGroupCode, ReportingFoodGroupDesc, .after = MainFoodGroupDesc)
```
...
Descriptions <- df.intake24_item %>%
  count(MainFoodGroupDesc, MainFoodGroupCode,Description_English, SubFoodGroupCode, ReportingFoodGroupDesc, ReportingFoodGroupCode, NutrientTableCode)
```
...
# **TO CONFIRM** Recalls with fewer than 5 items
```{r include=FALSE}
...
#remove recalls that didn't pass quality check
df.intake24_item <- df.intake24_item %>%
  filter(!UserID == 10013908) #Geraldine opts to keep this recall, need agreement here

df.survey <- df.survey %>%
  filter(!UserID == 10013908) #removing from survey as they only have 1 recall

df.intake24_item <- df.intake24_item %>%
  filter(!(UserID == 10006095 & RecallNo == 3)) #Geraldine opts to keep this recall, need agreement here

df.intake24_item <- df.intake24_item %>%
  filter(!(UserID == 10013179 & RecallNo == 2)) #unsure if Geraldine agrees on this exclusion
...
```{r eval=FALSE, include=FALSE}
write.csv(df.intake24_item, file = "/Users/ricki/Desktop/DISH_data_cleaning/Output/intake24_item_050924.csv")
write.csv(df.survey, file = "/Users/ricki/Desktop/DISH_data_cleaning/Output/survey_clean_050924.csv")
...
```{r}
check <- df.intake24_item %>%
  distinct(Description_English, NutrientTableCode)

write.csv(check, file = "/Users/ricki/Desktop/DISH_data_cleaning/Output/Intake24_items_060924.csv")
```
